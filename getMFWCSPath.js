function getMFWCSPath(ws,coverageID,limitesGeo,time,elevation){ // calcul du path pour une requete getCoverage du WCS
	...
	coverageID="BRIGHTNESS_TEMPERATURE__GROUND_OR_WATER_SURFACE___2016-12-10T00.00.00Z"; // constant pour les tests
	time="2016-12-10T12:00:00Z";										                // constant pour les tests
	...
	path+= '&coverageid='+coverageID;      		// le coverageId fourni par getDescribeCoveragenom de la couche � tracer
	path+= '&'+getSubsetGeo(limitesGeo);			// les subsets geographiques
	path+= '&subset=time('+time+')';            // heure de la pr�vision
	//path+= '&subset=pressure(850)';             // niveau (� pr�ciser en fonction du coverageId que l'on utilise)
	return path;
